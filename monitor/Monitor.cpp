...
void halo_monitor_loop(MonitorState &M, std::atomic<bool> &ShutdownRequested) {
  /////////
  // Setup

  // kick-off the chain of async read requests from the signal file descriptor.
  M.schedule_signalfd_read();

  while (!ShutdownRequested) {

    M.PerfSignalService.poll(); // check for new data
