...
mkdir build install
cd ./build
cmake -G "$GENERATOR" -DLLVM_USE_PERF=ON -DLLVM_ENABLE_PROJECTS="$PROJECTS" -DLLVM_TARGETS_TO_BUILD="$BACKENDS" -DLLVM_USE_LINKER=gold -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=../install ../src/llvm
$BUILD_CMD
